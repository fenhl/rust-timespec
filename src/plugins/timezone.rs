...
impl Plugin for WithTimeZone {
    fn parse(&self, plugins: &HashMap<String, Box<dyn Plugin>>, _: Tz, start: DateTime<Utc>, param_str: &str) -> Result<Predicate, Error> {
        ...
        shlex::split(spec).ok_or_else(|| Error::Plugin(format!("failed to split zoned timespec")))?.into_iter().map(|pred| Predicate::from_str_with_plugins(plugins, zone, &start.with_timezone(&zone), &pred)).collect::<Result<_, _>>().map(Predicate::All)
    }
}
